void verificarVolumetriaAgua() {
  ...
  TelnetStream.println("Executando função: verificarVolumetriaAgua()");
  ...
  if ((millis() - tempo_antes) > 1000 && digitalRead(pino_Trilha1_Solenoide) == HIGH) {
    ...
    fluxo = ((6000.0 / (millis() - tempo_antes)) * contador) / FATOR_CALIBRACAO;
    //exibicao do valor de fluxo
    Serial.print("Fluxo de: ");
    Serial.print(fluxo);
    Serial.println(" L/min");
    litrosPorMinuto_Trilha1 = fluxo;
    TelnetStream.println("Fluxo de: ");
    TelnetStream.println(fluxo);
    TelnetStream.println(" L/min. \n");
    ...
    Serial.println(" L");
    Serial.println();
    TelnetStream.println("Volume: ");
    TelnetStream.println(volume_total);
    TelnetStream.println(" L. \n");
